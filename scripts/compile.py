...
from PyKbd.wininf import generate_inf_file, generate_inf_launcher, get_name
...
print(f"Compiling {get_name(layout)}...")
...
with open(sys.argv[1] + "_install.cmd", "w", encoding="utf-8") as f:
    f.write(generate_inf_launcher(layout, sys.argv[1] + ".inf"))
with open(sys.argv[1] + "_uninstall.cmd", "w", encoding="utf-8") as f:
    f.write(generate_inf_launcher(layout, sys.argv[1] + ".inf", uninstall=True))
